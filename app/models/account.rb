class Account < ActiveRecord::Base
  ...
  attr_accessible :facebook_access_token, :twitter_api_secret, :twitter_api_key, :twitter_access_token, :twitter_access_token_secret
  ...
  def add_default_admin_user
    if User.find_by_email("merchant@gullakmaster.com").present?
      ...
      user = User.create(:email => "merchant@gullakmaster.com", :password => "gmkg2013", :password_confirmation => "gmkg2013", :first_name => "Abhishek", :last_name => "Dadoo", :mobile_number => "983000000000", :user_key => "abcdef", :is_active => true, :email_verified => true, :mobile_verified => false)
      self.users << user
    end
  end
